...
func (suite *RwApiTestSuite) TestPRDiscardOnBranchDelete() {
	t := suite.T()
	...
	cg.Must(t, "push", "origin", "--delete", "feature")

	suite.WaitForWorkflows(t, entities.WorkflowTypes.CalculateMergeConflict)

	pr, err = client.Get(ctx, &pb.GetPullRequestRequest{
	...
	require.NoError(t, err)
	require.NotNil(t, pr)
	require.Equal(t, pb.PullRequest_DISCARDED, pr.GetPrStatus())
}

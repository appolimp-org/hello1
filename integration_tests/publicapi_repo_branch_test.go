...
func (suite *RwApiTestSuite) TestPublicAPIListBranches() {
	t := suite.T()
	...
	tt := map[string]struct {
		...
		"happy_path": {
			url:      fmt.Sprintf("/repos/%s/%s/branches", suite.orgs.Yandex.Slug, suite.repos.ListBranches.Slug),
			user:     suite.users.Kopatych,
			expected: http.StatusOK,
		},
		"by_repo_id": {
			url:      fmt.Sprintf("/repos/by-id/%s/branches", suite.repos.ListBranches.UUID.String()),
			user:     suite.users.Kopatych,
			expected: http.StatusOK,
		},
		"by_repo_id_prefix": {
			url:      fmt.Sprintf("/repos/id:%s/branches", suite.repos.ListBranches.UUID.String()),
			user:     suite.users.Kopatych,
			expected: http.StatusOK,
		},
		"limit": {
		...
		"repo_not_found": {
			url:      fmt.Sprintf("/repos/%s/%s/branches", suite.orgs.Yandex.Slug, "nonexistent"),
			user:     suite.users.Kopatych,
			expected: http.StatusNotFound,
		},
		"forbidden": {
			url:      fmt.Sprintf("/repos/%s/%s/branches", suite.orgs.Yandex.Slug, suite.repos.AuthRepoPrivate.Slug),
			user:     suite.users.Kopatych,
	...
	t.Run("pagination", func(t *testing.T) {
		...
		t.Run("first", func(t *testing.T) {
			resp, err := suite.gwClient.As(suite.users.Kopatych.Identity).
				...
				Get(fmt.Sprintf("/repos/%s/%s/branches", suite.orgs.Yandex.Slug, suite.repos.ListBranches.Slug))
			require.NoError(t, err)
